...
class MLE():
    ...
    def optimized_log_likelihood(self,alpha, log_p_values, log_q_values):
        ...
        if (1 - alpha) * np.exp(log_p_values) + alpha * np.exp(log_q_values)==0:
            print()
    ...
    def bootstrap_alpha_inference(self,data,n_bootstrap=1000):
        ...
        if len(alpha_values_bootstrap)==0:
            return np.array([0, 0])
    ...
    def inference_change(self, sentence):
        """
        Performs inference on a single sentence, estimating the mixing parameter (alpha) and its confidence interval.

        This method processes a sentence to retain relevant tokens and uses bootstrap resampling to infer the mixing
        parameter alpha between human-generated and AI-generated text distributions. It calculates a confidence interval
        for alpha and prints the estimated value along with its precision.
        ...
        if not isinstance(sentence, list) or len(sentence) <= 1:
            raise ValueError("Input must be a valid token list with at least two token.")

        # Convert the sentence into a set of tokens found in the vocabulary
        tokens = set(token for token in sentence if token in self.all_tokens_set)

        # If no valid tokens are found, raise an error
        if not tokens:
            solution = -10
            half_width = 0
            return solution, half_width
            #raise ValueError("No valid tokens found in the sentence based on the vocabulary.")

        # Infer the confidence interval for the mixing parameter alpha
        confidence_interval = self.bootstrap_alpha_inference([tokens])

        # Calculate and round the mean of the confidence interval and its half-width
        solution = round(np.mean(confidence_interval), 3)
        half_width = round((confidence_interval[1] - confidence_interval[0]) / 2, 3)
